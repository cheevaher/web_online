  ...
  useEffect(() => {
    const fetchCourseAndPaymentStatus = async () => {
      try {
        ...
        // โหลดบทเรียนตัวอย่าง (preview) ซึ่ง backend ส่งมาเป็น object เดี่ยว
        const lessonRes = await fetch(`http://localhost:4000/api/courses/${id}/lessons/preview`);

        if (!lessonRes.ok) throw new Error('ບໍ່ສາມາດໂຫຼດບົດຮຽນໄດ້');

        const lessonData = await lessonRes.json();

        // แก้ไขที่นี่: backend ส่ง object ไม่ใช่ array
        if (lessonData) {
          setFirstLesson(lessonData);
        }
    ...
    <div className="max-w-5xl mx-auto px-6 py-12">
      ...
      <div className="space-y-6">
        ...
        <p className="text-base text-gray-500">
          ຜູ້ສອນ: <span className="font-semibold text-blue-700">{course.instructor_name}</span>
        </p>

        <p className="text-base text-gray-500">
          ຄອສນີ້ມີທັງໝົດ <span className="font-semibold text-blue-600">{lessonCount}</span> ວິດີໂອ
        </p>

        {firstLesson && (
          <div className="mb-8">
            <h2 className="text-xl font-semibold text-gray-800">ຕົວຢ່າງ: {firstLesson.lesson_name}</h2>
            <div className="aspect-w-16 aspect-h-9 mb-6">
              <iframe
                src={firstLesson.video_url}
                title={firstLesson.lesson_name}
                allowFullScreen
        ...
        <div className="flex items-center justify-between flex-wrap gap-4 bg-purple-50 p-6 rounded-xl shadow-inner">
          <div>
            ...
            <p className="text-blue-700 font-medium text-base">{course.category}</p>
          </div>
          <div>
            <p className="text-gray-600 text-sm">ລາຄາ</p>
            <p className="text-blue-700 font-bold text-xl">₭{course.price}</p>
          </div>
